...
export const GoalTracker = forwardRef<GoalTrackerRef, GoalTrackerProps>(({ onGoalUpdate }, ref) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (activeGoal) {
        localStorage.setItem('active-goal', activeGoal)
      }
    }
  }, [goals, activeGoal])
  ...
  const progress = activeGoalData 
    ? (activeGoalData.current / activeGoalData.target) * 100 
    : 0
  const dailyProgress = activeGoalData 
    ? (activeGoalData.dailyProgress / activeGoalData.dailyTarget) * 100 
    : 0

  // Expose methods via ref
  useImperativeHandle(ref, () => ({
    updateProgress: (increment = 1) => {
      return updateProgress(increment)
    },
    getProgress: () => {
    ...
    getActiveGoal: () => {
      ...
      return goals.find(g => g.id === activeGoal) || null
    }
  }))
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <h4 className="font-medium">{activeGoalData.title}</h4>
                      {activeGoalData.completed && (
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <span>
                          {activeGoalData.dailyProgress} / {activeGoalData.dailyTarget} {activeGoalData.unit}
                        </span>
                      </div>
